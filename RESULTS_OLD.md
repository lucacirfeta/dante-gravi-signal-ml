...
| Run | Session ID | GPS Start Interval | Start Date (UTC) | GPS End Interval | End Date (UTC) | Total Duration (Hours) |
|:----|:-----------|:-------------------|:-----------------|:-----------------|:---------------|:-----------------------|
| `O4a` | `20260520_223147` | `1382918784` | `2023-11-02 00:06:06` | `1384112128` | `2023-11-15 19:35:10` | `331.5` |
| `O4a` | `20260522_074026` | `1370206208` | `2023-06-07 20:49:50` | `1371395168` | `2023-06-21 15:05:50` | `330.3` |
| `O4a` | `20260523_143914` | `1385542816` | `2023-12-02 08:59:58` | `1386565632` | `2023-12-14 05:06:54` | `284.1` |
| `O4a` | `20260524_200219` | `1386797312` | `2023-12-16 21:28:14` | `1387994880` | `2023-12-30 18:07:42` | `332.7` |
| `O3a` | `20260530_014148` | `1238166048` | `2019-04-01 15:00:30` | `1238684480` | `2019-04-07 15:01:02` | `144.0` |
...
### 3.9.1 Metodologia
- **Dataset:** Segmenti di background reali L1 O4a + iniezioni sintetiche (n=30 per livello di ampiezza).
...
| `SpiralBurst`| **$68$** | **Quasi-Perfetta** ($KS \sim 0.905$, $p \sim 3 \times 10^{-17}$) | **16.6%** | Il KS-test dimostra un'integrazione morfologica riuscita ("bulk" separato). Tuttavia, le code pesanti del rumore distruggono la Recall. |

**Esito Finale:** La previsione era assolutamente esatta. La validazione empirica ha dimostrato che il Patch-Level Scoring aggira la Signal Dilution separando nettamente le masse delle distribuzioni (come testimoniato dall'eccellente score del KS-test). **Tuttavia**, la presenza di code pesantissime (Extreme Value) nel rumore di background O4a spinge il 99-esimo percentile a valori irraggiungibili per la maggioranza degli `SpiralBurst`. 
Il risultato finale è brutale: se si impone rigorosamente il tetto dell'1% ai Falsi Positivi garantendo un threshold empirico robusto, la Recall effettiva crolla miseramente al **16.6%** anche a SNR stellari ($\sim 138$).

Questo certifica che le code pesanti di LIGO limitano intrinsecamente il recupero dei segnali anomali anche se il modello di base è capace di separarne le distribuzioni medie. Invece di iterare verso soluzioni non supervisionate instabili (come Masked Autoencoders, ufficialmente bocciati per rischio di collapse e data leakage sul background stocastico), la strategia si muove verso la **Fase 4 (MIL Clustering):** raggruppare i pochi ma purissimi glitch NOVEL estratti con questa rigorosa soglia p99 tramite VLAD pooling e DPMM, validando se formino nuovi manifold fisici coerenti.

---

## 🔬 3.10 Phase 3 Extension: Topological Saliency Decoupling

Per interpretare visivamente le anomalie estratte al patch-level senza soffrire degli artefatti spettrali ai confini della *Q-Transform*, abbiamo sviluppato una **Topological Saliency Map**.

### 3.10.1 Decoupling dal Vector Quantized Index
L'indice globale VQ, essendo invariante spazialmente (le posizioni dei patch vengono perse durante il clustering *MiniBatchKMeans*), produce massicci falsi positivi ai bordi superiore e inferiore se usato per mappare la Saliency.
La soluzione adottata (`src/saliency_map.py`) consiste in un **disaccoppiamento architetturale**: la Saliency Map viene calcolata *esclusivamente* misurando la distanza coseno contro una **Spatial Median Matrix** `(1369, 384)` precalcolata pixel-by-pixel da segmenti nulli.

### 3.10.2 Analisi Fisica del Max/Mean Ratio
Testando la Saliency Map su iniezioni a SNR 135 e su puro rumore (No_Glitch), è emerso che:

| Campione | Ratio (Max/Mean) | Hotspot (row, col) | Status |
|:---|:---|:---|:---|
| Scattered_Light | 8.54 | (22, 11) | Localizzazione coerente |
| SpiralBurst (SNR=135) | 5.11 | (17, 23) | Localizzazione perfetta al centro |
| No_Glitch (Noise) | 5.54 | (8, 9) | Nessun segnale presente, ma fluttuazione geometrica severa |

**Conclusione Fisica:** In spazi ad alta dimensionalità come DINOv2, le code Fréchet del rumore stocastico si manifestano come picchi simili a delta di Dirac (alzando il Max), mentre i segnali fisicamente coerenti come SpiralBurst spalmano la loro energia (alzando il Mean). Il fatto che il rumore presenti un ratio (5.54) paragonabile o superiore a burst massicci (5.11) certifica che lo spazio ViT è severamente non-isotropico. 
Di conseguenza, le mappe di Saliency sono limitate a puri **visualizzatori topologici post-hoc** e non possono fungere da rilevatori globali su singoli frame, confermando l'autorità dell'indice VQ aggregato e della GEV per il detection primario. (Rif. Draft *Paper 3*).

...
- **Curve di efficienza per scala (2026-07-14; SUPERATE, registro storico):** le 1.200 injection per detector erano component-only e non sono evidenza primaria dopo il freeze v2. Restano immutate nei percorsi storici `results/micro_mdc/multiscale/`.
- **Efficienza multiscala v2 (2026-09-16; evidenza primaria verificata):** 7.440 injection appaiate su blocchi O4a disgiunti (6.000 nella popolazione primaria e 1.440 controlli DSD), decisione end-to-end a 32 s e quattro risposte corte esclusivamente condizionali. Su 276 celle condizionali definite, 250 (90,6%) sono agli endpoint osservati (140 sempre assenti, 110 sempre presenti); solo 26 sono intermedie. La sensibilità primaria è fortemente morfologia-dipendente: `NoiseBlob` non produce sostanziale risposta di score, `Whistle` produce una risposta crescente che resta quasi sempre sotto p99, e `Blip` cresce solo debolmente ad alto SNR; `NarrowChirp` e `ScatteredLight` forniscono controlli positivi con curve crescenti. `WallOfLines` non è mai recuperata in nessuno dei 12 bin detector/SNR da 8 a 48 (0/40 ciascuno). Un replay diretto su otto percorsi e l'audit completo degli hash escludono, entro 7,11e-15 sul target SNR, un difetto di scaling o injection immutata; il componente post-injection responsabile non è isolato. Intervalli al 95% da 2.000 bootstrap di blocchi raw completi; CI nullo agli endpoint e se una replica condizionale ha denominatore nullo. Nessuna fusione OR, upper limit di rate, significatività globale o validità O3. Report: `docs/MULTISCALE_EFFICIENCY_V2_REPORT.md`.
...
- **Falsifiability and Selectivity (Controlled Recovery Test; SUPERATO):** this earlier component-level study reported near-complete high-SNR recovery for its tested synthetic transients. It is retained only as historical context and is not evidence for current end-to-end efficiency. The paired v2 study above supersedes that interpretation and finds strong morphology dependence, including zero `WallOfLines` primary recovery throughout SNR 8--48.
...
*   **Commit Hash / Reference:** Please refer to the Zenodo deposition (DOI: 10.5281/zenodo.20960011) for the exact frozen artifacts (`master_report.json`, `global_taxonomy_report.json`) generated in `data/production/aggregated/`. **This is the deposition of the historical run described on this page, not the current one.** The artifacts backing the published V3 results are archived under DOI [10.5281/zenodo.21451803](https://doi.org/10.5281/zenodo.21451803) (git tag `3.5.0`).

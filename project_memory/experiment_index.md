...
| EXP-MDC-10 | Saliency Map Max/Mean Ratio (Spatial Decoupling) | L1 O4a Background + SpiralBurst + Scattered Light | Decoupled Saliency Map from the global VQ index. Computed anomaly purely against a Spatial Median Matrix (1369, 384). Evaluated `max/mean` ratio to measure relative hotspot strength. | **Scattered Light:** Perfect tracking (hotspot moved to low frequency). **No_Glitch:** Ratio 5.54. **SpiralBurst:** Ratio 5.11. **Conclusion:** ViT cosine space produces heavy tails even on noise; Saliency Maps are accurate topological visualizers but possess no independent detection/discrimination power on single frames. |

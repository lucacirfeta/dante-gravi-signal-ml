#!/usr/bin/env python3
"""Run or verify corrected detector-aware O4a native classification."""
...
from src.dante_light.o4a_corrected_native_thresholds import (  # noqa: E402
    DEFAULT_EXTERNAL_ROOT as DEFAULT_THRESHOLD_ROOT,
)
...
def main() -> int:
    ...
    parser.add_argument(
        "--native-external-root", type=Path, default=DEFAULT_NATIVE_ROOT
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--rescore-external-root", type=Path, default=DEFAULT_RESCORE_ROOT
    )
    parser.add_argument(
        "--threshold-external-root", type=Path, default=DEFAULT_THRESHOLD_ROOT
    )
    ...
    common = {
        ...
        "threshold_external_root": args.threshold_external_root.resolve(),
        "external_root": args.external_root.resolve(),
        "device": args.device,
    }
    if args.stage == "run":
        summary, run_dir = run_native_classification(**common)
    else:
        summary, run_dir = verify_native_classification(**common)

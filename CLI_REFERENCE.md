...
This is the **primary command for executing the O4a full discovery scan**.

* **Under the Hood (Processing Details):**
  1. Spawns the `patch-production` pipeline identically to the standalone command, natively enforcing `--resume`. It maintains state via HDF5 SWMR checkpoints.
  2. Each session dynamically extracts its own **local p99 threshold** and saves raw anomaly scores.
  3. Upon conclusion of each step, it autonomously invokes the `production-cluster` (density-based clustering) and `production-report` (visual validation) commands sequentially.
  
- *Accepts identical arguments as `patch-production`.*

### 5b. `aggregate-report` (Global Aggregator & Final Report)
Cross-session reducer. Must be run **after** the full `patch-analysis` scan is complete (or beforehand to pre-compute strict global thresholds). It computes the **Global Block-Bootstrap Threshold** across the entire observing run and writes the `Final_Discovery_Report.md`.
...
### 14. `download-all-references` (Recommended)
Batch-downloads Gravity Spy classification CSVs from Zenodo and builds in-domain reference indexes for each run/detector combination automatically. Files are saved with the proper naming convention (e.g. `indomain_o3b_h1.npz`) in `data/reference/`. 
**Note:** For O4a, since Gravity Spy labels are missing, you should run this command for `O3b` to build the historical training set for Domain Shift detection.

```bash
python main.py download-all-references --run O3b
```

### 15. `build-patch-reference`
Builds the Vector Quantization (VQ) spatial dictionary index (e.g. `patch_compressed_index_o3b.npz`) used for the Top-K spatial pooling.

```bash
python main.py build-patch-reference --images-dir data/reference/build/o3b_h1/ --output data/reference/patch_compressed_index_o3b.npz
```

### 16. `build-reference` (Legacy)
Starts the generic builder by extracting an embeddings index from labeled GPS timestamps. 
*Requires manual output naming.*

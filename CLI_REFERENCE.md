...
Clusters the 768D novel anomalies extracted during `patch-production`. This command operates strictly on the HDF5 output of Phase 4 without applying any PCA bottlenecks to preserve DINOv2 non-linear topology.

* **Under the Hood (Processing Details):**
  1. Opens the `novelties.h5` SWMR archive.
  2. Extracts the `mil_vectors` (768D) and rigorously enforces PyTorch L2-normalization on the entire set.
  3. Executes the Dirichlet Process Mixture Model (`BayesianGaussianMixture`) directly on the 768D space to calculate the topological likelihood of each segment belonging to a new cluster class.
  4. Projects the 768D space down to 2D using UMAP (cosine metric) strictly for visualization purposes.

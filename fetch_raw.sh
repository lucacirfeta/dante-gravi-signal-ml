...
usage() {
    ...
    echo "  -o: Output directory. Default: data/raw"
...
while [ $CURRENT_START -lt $END_GPS ]; do
    ...
    if [ -f "$OUTPUT_DIR/$FILENAME" ]; then
        ...
        # We search for a single GPS point to get the file containing it
        API_URL="https://gwosc.org/archive/links/$RUN/$DETECTOR/$CURRENT_START/$CURRENT_START/json/"
        FILE_URL=$(curl -s "$API_URL" | jq -r '.path[0]')

        if [ "$FILE_URL" != "null" ] && [ -n "$FILE_URL" ]; then
            # curl -L handles redirects, -C - handles resume
            curl -L -C - "$FILE_URL" -o "$OUTPUT_DIR/$FILENAME" --silent
            echo "OK"
        else

"""Frozen COINCIDENCE continuation for the canonical O4a provenance rerun."""

from __future__ import annotations

import copy
from contextlib import ExitStack, contextmanager
import json
import math
import os
from pathlib import Path
from typing import Any, Iterator, Mapping, Sequence

from src.core.index_contract import sha256_file
from src.dante_light import o4a_canonical_provenance_rerun as base
from src.dante_light import (
    o4a_canonical_native_classification_rerun as classification_remediation,
)
from src.dante_light import (
    o4a_canonical_native_taxonomy_rerun as taxonomy_remediation,
)
from src.dante_light.contracts import ContractError, canonical_json_sha256
from src.dante_light.o4a_corrected_native_rescore_v2 import _load_jsonl
from src.dante_light.o4a_native_provenance import verify_reference_with_reconciliation


ROOT = base.ROOT
AMENDMENT_REL = Path(
    "config/dante_o4a_canonical_provenance_coincidence_runtime_amendment_v1.json"
)
COINCIDENCE_EVIDENCE_REL = Path(
    "artifacts/dante_light/o4a_v1_parity/provenance_rerun_v1/"
    "corrected_native_coincidence.json"
)
HISTORICAL_EVIDENCE_REL = Path(
    "artifacts/dante_light/o4a_v1_parity/corrected_native_coincidence.json"
)
EXPECTED_AMENDMENT_DIGEST = (
    "df185633959ac0e02f552c4b0ac681e063d2c0d986247cefb8a502c46278bfaa"
)


def _read_json(path: Path) -> dict[str, Any]:
    if not path.is_file():
        raise ContractError(f"required JSON evidence is absent: {path}")
    value = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise ContractError(f"expected JSON object: {path}")
    return value


def load_runtime_amendment(
    *, root: Path = ROOT, require_current: bool = False
) -> dict[str, Any]:
    root = root.resolve()
    value = _read_json(root / AMENDMENT_REL)
    payload = dict(value)
    declared = payload.pop("amendment_digest", None)
    if declared != EXPECTED_AMENDMENT_DIGEST or declared != canonical_json_sha256(
        payload
    ):
        raise ContractError("COINCIDENCE runtime amendment digest mismatch")
    if (
        value.get("schema_version") != base.SCHEMA_VERSION
        or value.get("status") != "FROZEN_BEFORE_COINCIDENCE_RECOMPUTATION"
        or value.get("scope")
        != {"stage": "COINCIDENCE", "allowed_contract_changes": []}
    ):
        raise ContractError("COINCIDENCE runtime amendment scope changed")
    protocol_path = base._require_file_reference(
        root, value["parent_protocol"], label="COINCIDENCE amendment parent protocol"
    )
    if _read_json(protocol_path).get("protocol_digest") != value["parent_protocol"][
        "protocol_digest"
    ]:
        raise ContractError("COINCIDENCE amendment parent protocol mismatch")
    parent_path = base._require_file_reference(
        root,
        value["parent_runtime_amendment"],
        label="COINCIDENCE parent runtime amendment",
    )
    if _read_json(parent_path).get("amendment_digest") != value[
        "parent_runtime_amendment"
    ]["amendment_digest"]:
        raise ContractError("COINCIDENCE parent amendment digest mismatch")

    from src.dante_light.o4a_corrected_runtime import (
        validate_canonical_runtime_contract,
    )

    historical_path = base._require_file_reference(
        root, value["historical_runtime"], label="historical runtime contract"
    )
    remediation_path = base._require_file_reference(
        root, value["remediation_runtime"], label="remediation runtime contract"
    )
    historical = validate_canonical_runtime_contract(
        _read_json(historical_path), root=root, require_current=False
    )
    remediation = validate_canonical_runtime_contract(
        _read_json(remediation_path),
        root=root,
        require_current=require_current,
        device="cuda",
    )
    for label, runtime, reference in (
        ("historical", historical, value["historical_runtime"]),
        ("remediation", remediation, value["remediation_runtime"]),
    ):
        environment = runtime["runtime_environment"]
        if (
            runtime["contract_digest"] != reference["contract_digest"]
            or environment["environment_digest"] != reference["environment_digest"]
            or environment["cuda_device"]["driver_version"]
            != reference["driver_version"]
        ):
            raise ContractError(f"{label} COINCIDENCE runtime binding mismatch")
    if base.json_leaf_differences(
        historical["runtime_environment"], remediation["runtime_environment"]
    ) != set(value["required_environment_differences"]):
        raise ContractError("COINCIDENCE runtime amendment is not driver-only")
    if value.get("scientific_boundary") != {
        "candidate_population_changed": False,
        "measurement_or_null_changed": False,
        "native_index_payload_changed": False,
        "package_versions_changed": False,
        "partner_selection_changed": False,
        "scoring_changed": False,
        "seed_classes_changed": False,
        "threshold_rule_changed": False,
        "tolerances_changed": False,
    }:
        raise ContractError("COINCIDENCE amendment scientific boundary changed")
    return value


def _verify_reference(root: Path, reference: Mapping[str, Any], label: str) -> Path:
    path = base._inside_root(root, reference["path"], label=label)
    verify_reference_with_reconciliation(
        root=root,
        path=path,
        expected_sha256=str(reference["sha256"]),
        raw_hasher=sha256_file,
    )
    return path


def validate_contract(
    contract: Mapping[str, Any], *, root: Path = ROOT
) -> dict[str, Any]:
    root = root.resolve()
    candidate = json.loads(json.dumps(contract))
    if candidate.get("contract_digest") != base.contract_digest(candidate):
        raise ContractError("canonical COINCIDENCE contract digest mismatch")
    protocol = base.load_protocol(root=root, verify_git=True)
    stage = base.stage_spec(protocol, "COINCIDENCE")
    baseline = _read_json(root / stage["baseline_contract"]["path"])
    base.assert_allowed_contract_transition(
        baseline, candidate, allowed_changes=stage["allowed_changes"]
    )
    for name, reference in candidate.get("references", {}).items():
        _verify_reference(root, reference, f"COINCIDENCE {name}")
    for name in ("historical_coincidence_evidence", "delegated_implementation"):
        _verify_reference(
            root,
            candidate["remediation"][name],
            f"COINCIDENCE remediation {name}",
        )
    remediation = candidate.get("remediation", {})
    if (
        remediation.get("historical_outputs_immutable") is not True
        or remediation.get("classification_ledger_byte_identical") is not True
        or remediation.get("index_scientific_payload_byte_identical") is not True
        or remediation.get("taxonomy_ledger_byte_identical") is not True
        or remediation.get("pem_computed") is not False
    ):
        raise ContractError("COINCIDENCE remediation boundary changed")
    return candidate


def _external_path(value: str) -> Path:
    return classification_remediation._external_path(value)


def _verified_evidence(
    path: Path,
    *,
    status: str,
    comparison_key: str,
    accepted_comparison: str,
) -> dict[str, Any]:
    evidence = _read_json(path)
    if (
        evidence.get("status") != status
        or evidence.get("comparison_to_historical", {}).get(comparison_key)
        != accepted_comparison
    ):
        raise ContractError(f"upstream evidence is not verified: {path}")
    return evidence


def _verified_external_summary(evidence: Mapping[str, Any]) -> tuple[dict[str, Any], Path]:
    external = evidence["external_run"]
    path = _external_path(str(external["directory"])) / str(
        external["summary_filename"]
    )
    if sha256_file(path) != external.get("summary_sha256"):
        raise ContractError(f"upstream external summary hash changed: {path}")
    return _read_json(path), path


def build_contract(*, root: Path = ROOT) -> dict[str, Any]:
    root = root.resolve()
    protocol = base.load_protocol(root=root, verify_git=True)
    stage = base.stage_spec(protocol, "COINCIDENCE")
    baseline = _read_json(root / stage["baseline_contract"]["path"])

    classification_path = root / classification_remediation.CLASSIFICATION_EVIDENCE_REL
    classification = _verified_evidence(
        classification_path,
        status="PASS_VERIFIED_CANONICAL_CLASSIFICATION",
        comparison_key="classification",
        accepted_comparison="BYTE_IDENTICAL",
    )
    classification_summary, _ = _verified_external_summary(classification)
    if classification_summary.get("output") != classification.get("output"):
        raise ContractError("canonical CLASSIFY output binding changed")

    index_path = root / (
        "artifacts/dante_light/o4a_v1_parity/provenance_rerun_v1/"
        "corrected_native_index.json"
    )
    index = _read_json(index_path)
    if (
        index.get("status") != "PASS_VERIFIED_CANONICAL_INDEX"
        or index.get("comparison_to_historical", {}).get("classification")
        != "SCIENTIFIC_PAYLOAD_BYTE_IDENTICAL"
        or index.get("comparison_to_historical", {}).get("centroid_bytes_equal")
        is not True
        or index.get("comparison_to_historical", {}).get("raw_sample_bytes_equal")
        is not True
    ):
        raise ContractError("canonical INDEX scientific payload is not byte-identical")

    taxonomy_path = root / taxonomy_remediation.TAXONOMY_EVIDENCE_REL
    taxonomy = _verified_evidence(
        taxonomy_path,
        status="PASS_VERIFIED_CANONICAL_TAXONOMY",
        comparison_key="classification",
        accepted_comparison="BYTE_IDENTICAL",
    )
    taxonomy_summary, _ = _verified_external_summary(taxonomy)
    if taxonomy_summary.get("output") != taxonomy.get("output"):
        raise ContractError("canonical TAXONOMY output binding changed")

    primary = _read_json(root / baseline["references"]["primary_scan"]["path"])
    amendment = load_runtime_amendment(root=root, require_current=False)
    candidate = copy.deepcopy(baseline)
    candidate["contract_id"] = "dante-o4a-canonical-provenance-rerun-coincidence-v1"
    candidate["parents"] = {
        "native_classification_artifact_digest": classification[
            "external_artifact_digest"
        ],
        "native_classification_contract_digest": classification["contract_digest"],
        "native_classification_row_digest": classification["output"]["row_digest"],
        "native_classification_sha256": classification["output"]["sha256"],
        "native_index_artifact_digest": index["external_artifact_digest"],
        "native_index_contract_digest": index["contract_digest"],
        "native_index_sha256": index["index"]["sha256"],
        "native_taxonomy_artifact_digest": taxonomy["external_artifact_digest"],
        "primary_scan_artifact_digest": primary["artifact_digest"],
        "primary_scan_database_sha256": primary["database"]["sha256"],
    }
    candidate["references"]["native_classification"] = {
        "path": classification_remediation.CLASSIFICATION_EVIDENCE_REL.as_posix(),
        "sha256": sha256_file(classification_path),
    }
    candidate["references"]["native_index"] = {
        "path": index_path.relative_to(root).as_posix(),
        "sha256": sha256_file(index_path),
    }
    candidate["references"]["native_taxonomy"] = {
        "path": taxonomy_remediation.TAXONOMY_EVIDENCE_REL.as_posix(),
        "sha256": sha256_file(taxonomy_path),
    }
    candidate["references"]["implementation"] = {
        "path": "src/dante_light/o4a_canonical_native_coincidence_rerun.py",
        "sha256": sha256_file(Path(__file__).resolve()),
    }
    candidate["output"]["root"] = protocol["paths"]["remediation_external_roots"][7]
    candidate["remediation"] = {
        "protocol_digest": protocol["protocol_digest"],
        "baseline_contract_sha256": stage["baseline_contract"]["sha256"],
        "runtime_amendment": {
            "path": AMENDMENT_REL.as_posix(),
            "digest": amendment["amendment_digest"],
        },
        "runtime_environment_digest": amendment["remediation_runtime"][
            "environment_digest"
        ],
        "historical_coincidence_evidence": {
            "path": HISTORICAL_EVIDENCE_REL.as_posix(),
            "sha256": sha256_file(root / HISTORICAL_EVIDENCE_REL),
        },
        "delegated_implementation": {
            "path": "src/dante_light/o4a_corrected_native_coincidence.py",
            "sha256": sha256_file(
                root / "src/dante_light/o4a_corrected_native_coincidence.py"
            ),
        },
        "historical_outputs_immutable": True,
        "classification_ledger_byte_identical": True,
        "index_scientific_payload_byte_identical": True,
        "index_container_metadata_changed_only": True,
        "taxonomy_ledger_byte_identical": True,
        "primary_scan_immutable": True,
        "pem_computed": False,
    }
    candidate["contract_digest"] = base.contract_digest(candidate)
    return validate_contract(candidate, root=root)


def write_frozen_contract(*, root: Path = ROOT) -> Path:
    protocol = base.load_protocol(root=root, verify_git=True)
    target = base._inside_root(
        root,
        base.stage_spec(protocol, "COINCIDENCE")["remediation_contract"],
        label="COINCIDENCE remediation contract",
    )
    candidate = build_contract(root=root)
    serialized = json.dumps(candidate, indent=2, sort_keys=True, allow_nan=False) + "\n"
    if target.is_file():
        if target.read_text(encoding="utf-8") != serialized:
            raise ContractError(f"refusing divergent frozen contract: {target}")
        return target
    target.parent.mkdir(parents=True, exist_ok=True)
    temporary = target.with_name(f".{target.name}.{os.getpid()}.tmp")
    temporary.write_text(serialized, encoding="utf-8", newline="\n")
    temporary.replace(target)
    return target


def _frozen_contract(*, root: Path) -> dict[str, Any]:
    protocol = base.load_protocol(root=root, verify_git=True)
    path = base._inside_root(
        root,
        base.stage_spec(protocol, "COINCIDENCE")["remediation_contract"],
        label="COINCIDENCE remediation contract",
    )
    if not path.is_file():
        raise ContractError("COINCIDENCE remediation contract is not frozen")
    return validate_contract(_read_json(path), root=root)


def _load_runtime(
    *, root: Path, require_current: bool, device: str
) -> dict[str, Any]:
    amendment = load_runtime_amendment(root=root, require_current=False)
    reference = amendment["remediation_runtime"]
    from src.dante_light.o4a_corrected_runtime import (
        validate_canonical_runtime_contract,
    )

    return validate_canonical_runtime_contract(
        _read_json(root / reference["path"]),
        root=root,
        require_current=require_current,
        device=device,
    )


def _canonical_external_paths(
    *,
    root: Path,
    contract: Mapping[str, Any],
    primary_external_root: Path,
    classification_external_root: Path,
    index_external_root: Path,
) -> tuple[Path, Path, Path, Path]:
    classification = _read_json(
        root / contract["references"]["native_classification"]["path"]
    )
    primary = _read_json(root / contract["references"]["primary_scan"]["path"])
    index = _read_json(root / contract["references"]["native_index"]["path"])
    classification_dir = classification_external_root.resolve() / (
        "native_classification_" + classification["run_key"]
    )
    primary_dir = primary_external_root.resolve() / (
        "primary_scan_" + primary["run_key"]
    )
    index_dir = index_external_root.resolve() / ("native_index_" + index["run_key"])
    return (
        classification_dir / classification["output"]["filename"],
        classification_dir / classification["external_run"]["summary_filename"],
        primary_dir / primary["database"]["filename"],
        index_dir / index["external_run"]["index_filename"],
    )


@contextmanager
def _patched_module(
    *, root: Path, contract: Mapping[str, Any], device: str
) -> Iterator[Any]:
    from src.dante_light import o4a_corrected_native_coincidence as module

    runtime = _load_runtime(root=root, require_current=True, device=device)

    def load_contract(_root: Path = ROOT) -> dict[str, Any]:
        del _root
        return copy.deepcopy(dict(contract))

    def load_runtime(
        *, root: Path = ROOT, require_current: bool = False, device: str = "cuda"
    ) -> dict[str, Any]:
        del root, require_current, device
        return copy.deepcopy(runtime)

    with ExitStack() as stack:
        for attribute, value in (
            ("load_native_coincidence_contract", load_contract),
            ("load_canonical_runtime_contract", load_runtime),
            ("_external_paths", _canonical_external_paths),
        ):
            stack.enter_context(base.use_module_attribute(module, attribute, value))
        yield module


def run(
    *,
    root: Path = ROOT,
    device: str = "cuda",
    workers: int = 8,
    batch_size: int = 32,
    verify_only: bool = False,
) -> tuple[dict[str, Any], Path]:
    root = root.resolve()
    protocol = base.load_protocol(root=root, verify_git=True)
    contract = _frozen_contract(root=root)
    roots = protocol["paths"]["remediation_external_roots"]
    common = {
        "root": root,
        "raw_root": Path(protocol["paths"]["raw_root_wsl"]),
        "primary_external_root": Path(protocol["paths"]["primary_external_root_wsl"]),
        "classification_external_root": Path(roots[5]),
        "index_external_root": Path(roots[1]),
        "external_root": Path(roots[7]),
        "device": device,
    }
    with _patched_module(root=root, contract=contract, device=device) as module:
        if verify_only:
            return module.verify_native_coincidence(**common)
        return module.run_native_coincidence(
            **common, workers=workers, batch_size=batch_size
        )


def _rows_equivalent(
    current: Sequence[Mapping[str, Any]],
    historical: Sequence[Mapping[str, Any]],
    *,
    tolerance: float,
) -> bool:
    if len(current) != len(historical):
        return False
    allowed_float_fields = ("seed_replay_score", "seed_score_delta")
    for left, right in zip(current, historical, strict=True):
        left_copy = dict(left)
        right_copy = dict(right)
        for field in allowed_float_fields:
            left_value = left_copy.pop(field, None)
            right_value = right_copy.pop(field, None)
            if (
                left_value is None
                or right_value is None
                or not math.isfinite(float(left_value))
                or not math.isfinite(float(right_value))
                or abs(float(left_value) - float(right_value)) > tolerance
            ):
                return False
        if left_copy != right_copy:
            return False
    return True


def compare_to_historical(
    *, root: Path, summary: Mapping[str, Any], run_dir: Path
) -> dict[str, Any]:
    contract = _frozen_contract(root=root)
    historical_path = _verify_reference(
        root,
        contract["remediation"]["historical_coincidence_evidence"],
        "historical COINCIDENCE evidence",
    )
    historical = _read_json(historical_path)
    if historical.get("status") != "PASS_VERIFIED_NATIVE_COINCIDENCE_V1":
        raise ContractError("historical COINCIDENCE evidence is not PASS")
    historical_dir = _external_path(historical["external_run"]["directory"])
    historical_summary_path = historical_dir / historical["external_run"][
        "summary_filename"
    ]
    if (
        sha256_file(historical_summary_path)
        != historical["external_run"]["summary_sha256"]
    ):
        raise ContractError("historical COINCIDENCE summary hash changed")
    historical_summary = _read_json(historical_summary_path)
    tolerance = float(contract["gates"]["maximum_seed_score_delta"])
    checks: dict[str, bool] = {}
    for name in ("primary", "diagnostic", "raw_source_receipt"):
        current_path = run_dir / summary["outputs"][name]["filename"]
        old_path = historical_dir / historical["outputs"][name]["filename"]
        if sha256_file(old_path) != historical["outputs"][name]["sha256"]:
            raise ContractError(f"historical COINCIDENCE {name} hash changed")
        checks[f"{name}_bytes_equal"] = current_path.read_bytes() == old_path.read_bytes()
        if name != "raw_source_receipt":
            checks[f"{name}_numerically_equivalent"] = _rows_equivalent(
                _load_jsonl(current_path),
                _load_jsonl(old_path),
                tolerance=tolerance,
            )
    checks.update(
        {
            "event_summary_equal": summary["event_summary"]
            == historical_summary["event_summary"],
            "historical_anchor_equal": summary["historical_anchor_check"]
            == historical_summary["historical_anchor_check"],
            "population_equal": summary["population"] == historical_summary["population"],
            "measurement_equal": summary["measurement"]
            == historical_summary["measurement"],
            "classification_source_equal": (
                summary["sources"]["classification_row_digest"]
                == historical_summary["sources"]["classification_row_digest"]
            ),
            "primary_database_source_equal": (
                summary["sources"]["primary_database_sha256"]
                == historical_summary["sources"]["primary_database_sha256"]
            ),
            "raw_source_receipt_equal": (
                summary["sources"]["raw_source_receipt_digest"]
                == historical_summary["sources"]["raw_source_receipt_digest"]
            ),
        }
    )
    byte_equal = all(
        checks[name]
        for name in (
            "primary_bytes_equal",
            "diagnostic_bytes_equal",
            "raw_source_receipt_bytes_equal",
        )
    ) and all(
        checks[name]
        for name in (
            "event_summary_equal",
            "historical_anchor_equal",
            "population_equal",
            "measurement_equal",
            "classification_source_equal",
            "primary_database_source_equal",
            "raw_source_receipt_equal",
        )
    )
    numeric_equal = all(checks.values())
    classification = (
        "BYTE_IDENTICAL"
        if byte_equal
        else (
            "NUMERICALLY_EQUIVALENT_WITHIN_FROZEN_TOLERANCE"
            if numeric_equal
            else "OUTPUT_CHANGED"
        )
    )
    return {
        "classification": classification,
        "all_coincidence_outputs_identical": byte_equal,
        "all_outputs_within_frozen_tolerance": numeric_equal,
        "frozen_score_tolerance": tolerance,
        "checks": checks,
        "expected_index_container_change": {
            "historical_sha256": historical_summary["sources"][
                "native_index_sha256"
            ],
            "canonical_sha256": summary["sources"]["native_index_sha256"],
            "scientific_payload_byte_identical": True,
        },
    }


def write_verified_evidence(*, root: Path = ROOT, device: str = "cuda") -> Path:
    root = root.resolve()
    summary, run_dir = run(root=root, device=device, verify_only=True)
    contract = _frozen_contract(root=root)
    comparison = compare_to_historical(root=root, summary=summary, run_dir=run_dir)
    if comparison["classification"] == "OUTPUT_CHANGED":
        raise ContractError("canonical COINCIDENCE output changed")
    protocol = base.load_protocol(root=root, verify_git=True)
    contract_path = root / base.stage_spec(protocol, "COINCIDENCE")[
        "remediation_contract"
    ]
    summary_path = run_dir / contract["output"]["summary_filename"]
    body = {
        "schema_version": base.SCHEMA_VERSION,
        "status": "PASS_VERIFIED_CANONICAL_COINCIDENCE",
        "run_key": summary["run_key"],
        "contract_digest": summary["contract_digest"],
        "external_artifact_digest": summary["artifact_digest"],
        "runtime_environment_digest": summary["runtime_environment_digest"],
        "population": summary["population"],
        "measurement": summary["measurement"],
        "scientific_boundary": summary["scientific_boundary"],
        "event_summary": summary["event_summary"],
        "historical_anchor_check": summary["historical_anchor_check"],
        "sources": summary["sources"],
        "outputs": summary["outputs"],
        "gates": summary["gates"],
        "comparison_to_historical": comparison,
        "external_run": {
            "directory": os.fspath(run_dir),
            "summary_filename": summary_path.name,
            "summary_sha256": sha256_file(summary_path),
            "summary_size_bytes": summary_path.stat().st_size,
        },
        "references": {
            "contract": {
                "path": contract_path.relative_to(root).as_posix(),
                "sha256": sha256_file(contract_path),
            },
            "implementation": contract["references"]["implementation"],
            "runtime_amendment": {
                "path": AMENDMENT_REL.as_posix(),
                "sha256": sha256_file(root / AMENDMENT_REL),
                "digest": EXPECTED_AMENDMENT_DIGEST,
            },
        },
        "verification": {
            "independent_verifier": "PASS_EXACT_LEDGER_AND_DIGEST_REPLAY",
            "failure_artifact_absent": not (run_dir / "failure.json").is_file(),
            "pem_computed": False,
        },
    }
    evidence = {**body, "artifact_digest": canonical_json_sha256(body)}
    target = root / COINCIDENCE_EVIDENCE_REL
    base._atomic_json(target, evidence)
    return target


__all__ = [
    "AMENDMENT_REL",
    "COINCIDENCE_EVIDENCE_REL",
    "EXPECTED_AMENDMENT_DIGEST",
    "build_contract",
    "compare_to_historical",
    "load_runtime_amendment",
    "run",
    "validate_contract",
    "write_frozen_contract",
    "write_verified_evidence",
]

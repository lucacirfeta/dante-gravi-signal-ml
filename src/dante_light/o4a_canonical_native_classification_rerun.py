"""Frozen CLASSIFY continuation for the canonical O4a provenance rerun."""
...
ROOT = base.ROOT
AMENDMENT_REL = Path(
    "config/dante_o4a_canonical_provenance_classification_runtime_amendment_v1.json"
)
CLASSIFICATION_EVIDENCE_REL = Path(
    "artifacts/dante_light/o4a_v1_parity/provenance_rerun_v1/"
    "corrected_native_classification.json"
)
HISTORICAL_EVIDENCE_REL = Path(
    "artifacts/dante_light/o4a_v1_parity/corrected_native_classification.json"
)
EXPECTED_AMENDMENT_DIGEST = (
    "99364392627aca14e1621cd08037b80e76cdba4c9ac2b38ce65c74f83a946edf"
)
...
def load_runtime_amendment(
    *, root: Path = ROOT, require_current: bool = False
    ...
    if declared != EXPECTED_AMENDMENT_DIGEST or declared != canonical_json_sha256(
        ...
        raise ContractError("CLASSIFY runtime amendment digest mismatch")
    if (
        value.get("schema_version") != base.SCHEMA_VERSION
        or value.get("status") != "FROZEN_BEFORE_CLASSIFICATION_RECOMPUTATION"
        or value.get("scope")
        != {"stage": "CLASSIFY", "allowed_contract_changes": []}
    ):
        raise ContractError("CLASSIFY runtime amendment scope changed")
    protocol_path = base._require_file_reference(
        root, value["parent_protocol"], label="CLASSIFY amendment parent protocol"
    )
    if _read_json(protocol_path).get("protocol_digest") != value["parent_protocol"][
        "protocol_digest"
    ]:
        raise ContractError("CLASSIFY amendment parent protocol mismatch")
    parent_path = base._require_file_reference(
        root,
        value["parent_runtime_amendment"],
        label="CLASSIFY parent runtime amendment",
    )
    if _read_json(parent_path).get("amendment_digest") != value[
        "parent_runtime_amendment"
    ]["amendment_digest"]:
        raise ContractError("CLASSIFY parent amendment digest mismatch")
    ...
    for label, runtime, reference in (
        ...
        if (
            ...
            raise ContractError(f"{label} CLASSIFY runtime binding mismatch")
    if base.json_leaf_differences(
        ...
        raise ContractError("CLASSIFY runtime amendment is not driver-only")
    if value.get("scientific_boundary") != {
        ...
        raise ContractError("CLASSIFY amendment scientific boundary changed")
    return value
...
def validate_contract(
    ...
    if candidate.get("contract_digest") != base.contract_digest(candidate):
        raise ContractError("canonical CLASSIFY contract digest mismatch")
    protocol = base.load_protocol(root=root, verify_git=True)
    stage = base.stage_spec(protocol, "CLASSIFY")
    ...
    for name, reference in candidate.get("references", {}).items():
        _verify_reference(root, reference, f"CLASSIFY {name}")
    for name in ("historical_classification_evidence", "delegated_implementation"):
        _verify_reference(
            root, candidate["remediation"][name], f"CLASSIFY remediation {name}"
        )
    remediation = candidate.get("remediation", {})
    if (
        remediation.get("historical_outputs_immutable") is not True
        or remediation.get("taxonomy_computed") is not False
        or remediation.get("coincidence_computed") is not False
        or remediation.get("pem_computed") is not False
    ):
        raise ContractError("CLASSIFY remediation boundary changed")
    return candidate


def _external_path(value: str) -> Path:
    return thresholds_remediation._external_path(value)


def _verified_compact_evidence(
    path: Path, *, status: str, comparison: str
    ...
    if (
        ...
        raise ContractError(f"upstream evidence is not verified identical: {path}")
    ...
    if sha256_file(summary_path) != external["summary_sha256"]:
        raise ContractError(f"upstream external summary hash changed: {summary_path}")
    return evidence, _read_json(summary_path)
...
def build_contract(*, root: Path = ROOT) -> dict[str, Any]:
    ...
    stage = base.stage_spec(protocol, "CLASSIFY")
    baseline = _read_json(root / stage["baseline_contract"]["path"])
    thresholds_path = root / thresholds_remediation.THRESHOLDS_EVIDENCE_REL
    thresholds, thresholds_summary = _verified_compact_evidence(
        thresholds_path,
        status="PASS_VERIFIED_CANONICAL_THRESHOLDS",
        comparison="SCIENTIFIC_OUTPUT_IDENTICAL",
    )
    rescore_path = root / rescore_remediation.RESCORE_EVIDENCE_REL
    rescore = _read_json(rescore_path)
    if (
        rescore.get("status") != "PASS_VERIFIED_CANONICAL_RESCORE"
        or rescore.get("comparison_to_historical", {}).get("classification")
        != "BYTE_IDENTICAL"
    ):
        raise ContractError("canonical RESCORE evidence is not byte-identical")
    candidate_output = rescore["outputs"]["primary_candidate"]
    amendment = load_runtime_amendment(root=root, require_current=False)

    candidate = copy.deepcopy(baseline)
    candidate["contract_id"] = "dante-o4a-canonical-provenance-rerun-classification-v1"
    candidate["parent_native_thresholds"] = {
        "compact_artifact_digest": thresholds["artifact_digest"],
        "run_artifact_digest": thresholds["external_artifact_digest"],
        "contract_digest": thresholds["contract_digest"],
        "run_key": thresholds["run_key"],
        "summary_sha256": thresholds["external_run"]["summary_sha256"],
    }
    candidate["parent_native_rescore"] = {
        "compact_artifact_digest": rescore["artifact_digest"],
        "run_artifact_digest": thresholds_summary["native_rescore_artifact_digest"],
        "contract_digest": rescore["contract_digest"],
        "run_key": rescore["run_key"],
        "candidate_sha256": candidate_output["sha256"],
        "candidate_row_digest": candidate_output["row_digest"],
    }
    candidate["references"]["native_thresholds"] = {
        "path": thresholds_remediation.THRESHOLDS_EVIDENCE_REL.as_posix(),
        "sha256": sha256_file(thresholds_path),
    }
    candidate["references"]["native_rescore_v2"] = {
        "path": rescore_remediation.RESCORE_EVIDENCE_REL.as_posix(),
        "sha256": sha256_file(rescore_path),
    }
    candidate["references"]["implementation"] = {
        "path": "src/dante_light/o4a_canonical_native_classification_rerun.py",
        "sha256": sha256_file(Path(__file__).resolve()),
    }
    candidate["output"]["root"] = protocol["paths"]["remediation_external_roots"][5]
    candidate["remediation"] = {
        ...
        "historical_classification_evidence": {
            "path": HISTORICAL_EVIDENCE_REL.as_posix(),
            "sha256": sha256_file(root / HISTORICAL_EVIDENCE_REL),
        },
        "delegated_implementation": {
            "path": "src/dante_light/o4a_corrected_native_classification.py",
            "sha256": sha256_file(
                root / "src/dante_light/o4a_corrected_native_classification.py"
            ),
        },
        "historical_outputs_immutable": True,
        "rescore_ledgers_byte_identical": True,
        "thresholds_scientifically_identical": True,
...
def write_frozen_contract(*, root: Path = ROOT) -> Path:
    ...
    target = base._inside_root(
        root,
        base.stage_spec(protocol, "CLASSIFY")["remediation_contract"],
        label="CLASSIFY remediation contract",
    )
...
def _frozen_contract(*, root: Path) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise ContractError("CLASSIFY remediation contract is not frozen")
...
def _patched_module(
    *, root: Path, contract: Mapping[str, Any], device: str
    ...
    from src.dante_light import o4a_corrected_native_classification as module

    thresholds_summary, _ = thresholds_remediation.run(
    ...
    def verify_thresholds(**_kwargs: Any) -> tuple[dict[str, Any], Path]:
        return thresholds_summary, Path(contract["output"]["root"])
    ...
    with ExitStack() as stack:
        for attribute, value in (
            ("load_native_classification_contract", load_contract),
            ("verify_native_thresholds", verify_thresholds),
...
def run(
    *, root: Path = ROOT, device: str = "cuda", verify_only: bool = False
    ...
    common = {
        ...
        "native_external_root": Path(roots[0]),
        "index_external_root": Path(roots[1]),
        "calibration_external_root": Path(roots[2]),
        "rescore_external_root": Path(roots[3]),
        "threshold_external_root": Path(roots[4]),
        "external_root": Path(roots[5]),
        "device": device,
    }
    with _patched_module(root=root, contract=contract, device=device) as module:
        if verify_only:
            return module.verify_native_classification(**common)
        return module.run_native_classification(**common)
...
def compare_to_historical(
    *, root: Path, summary: Mapping[str, Any], run_dir: Path
    ...
    historical_path = _verify_reference(
        root,
        contract["remediation"]["historical_classification_evidence"],
        "historical CLASSIFY evidence",
    )
    ...
    if sha256_file(historical_output) != historical["output"]["sha256"]:
        raise ContractError("historical CLASSIFY output hash changed")
    checks = {
    ...
    return {
        ...
        "all_classification_outputs_identical": all_equal,
        "checks": checks,
    }
...
def write_verified_evidence(*, root: Path = ROOT, device: str = "cuda") -> Path:
    ...
    counts_by_class = {
        label: sum(
            detector_counts[label]
            for detector_counts in summary["counts_by_detector_and_class"].values()
        )
        for label in ("BACKGROUND", "AMBIGUOUS", "ROBUST")
    }
    protocol = base.load_protocol(root=root, verify_git=True)
    contract_path = root / base.stage_spec(protocol, "CLASSIFY")[
    ...
    body = {
        ...
        "native_threshold_artifact_digest": summary[
            "native_threshold_artifact_digest"
        ],
        "native_rescore_artifact_digest": summary["native_rescore_artifact_digest"],
        ...
        "counts_by_detector_and_class": summary["counts_by_detector_and_class"],
        ...
        "verification": {
            ...
            "taxonomy_or_coincidence_or_pem_computed": False,
        },
    }
    evidence = {**body, "artifact_digest": canonical_json_sha256(body)}
    target = root / CLASSIFICATION_EVIDENCE_REL

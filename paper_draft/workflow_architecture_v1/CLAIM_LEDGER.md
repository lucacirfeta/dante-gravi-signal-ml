...
| C34 | VERIFIED | The productization release adopts the corrected 10,942-candidate native classification (5,406 ROBUST, 2,344 AMBIGUOUS, 3,192 BACKGROUND); the 10,429-row v6 catalogue is preserved only as the historical comparison baseline. | `artifacts/dante_workflow/productization_v1_release.json`; `config/dante_o4a_corrected_native_classification_v1.json`; `config/dante_o4a_corrected_native_coincidence_v1.json`; `config/dante_o4a_corrected_final_comparison_v2.json` |
| C35 | VERIFIED | The 104/103+1 and 121/120+1 test counts belong to two source revisions; the latter is one 121-test release corpus with a single Windows-only test skipped on WSL. | `artifacts/dante_workflow/public_smoke_ui_checkpoint_2026-09-06.json`; `docs/DANTE_WORKFLOW_PRODUCTIZATION_RESULT.md`; `paper_draft/workflow_architecture_v1/EVIDENCE_AUDIT.md` |
| C36 | VERIFIED | Content-aware reuse is prior art: Snakemake and Nextflow provide provenance-sensitive job/task reuse, while Git, Bazel, and Nix provide relevant content-addressed storage/build precedents. The paper claims novelty only for the tested domain-specific composition. | Official documentation cited in `references.bib`; bounded comparison in `main.tex` |
| C37 | VERIFIED | The O4a-specific GWOSC release provides H1/L1 strain, checksum, segment, and auxiliary-data resources and identifies the released calibrated-strain channels. | GWOSC O4a data release, DOI `10.7935/kt51-6n86`, cited in `references.bib` |

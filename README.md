...
**Within the DINOv2 latent representation and strict Data Quality gating framework (`CBC_CAT1`) used in this study, 3 true astrophysical novelty candidates were discovered in the analyzed pristine O4a data.** Out of 183 anomalous prototypes generated dynamically without supervision, 180 were successfully matched against known O3b reference indexes, leaving exactly 3 orphaned transients during LVK Science Mode. No candidates were rejected by DQ criteria. To confirm their physical origin, a rigorous coincidence check against H1 and GWTC-4.0 revealed them to be non-bilateral local L1 glitches (DetChar findings). This validates the pipeline's robustness: separating the "bulk" of structural anomalies from true uncatalogued Out-Of-Distribution events, isolating rare transients for manual review.
